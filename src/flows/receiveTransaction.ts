        ...
        if (ready) {
            ...
            switch (coinsConfirmed) {
                ...
                case '00':
                    ...
                    return 0;
                default:
                    logger.info(`From Device (Unknown Value Received) : ${coinsConfirmed}`);
                    connection.close();
                    return 0;
            }

            logger.info(`Please verify if this is the same address on the device?\n${receiveAddress}`);

            const addressesVerified = await receiveCommand(connection, 63);

            logger.info(`From Device (Verified receive address) : ${addressesVerified}`);
